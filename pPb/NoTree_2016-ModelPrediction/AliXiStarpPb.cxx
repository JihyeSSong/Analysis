...
ClassImp(AliXiStarpPb)

//________________________________________________________________________
AliXiStarpPb::AliXiStarpPb():
AliAnalysisTaskSE(),
fname(0),
fESD(0x0),
fOutputList(0x0),
...
//________________________________________________________________________
AliXiStarpPb::AliXiStarpPb(const char *name, Bool_t AODdecision,  Bool_t MCdecision, Int_t CutListOption)
...
//________________________________________________________________________
AliXiStarpPb::AliXiStarpPb(const AliXiStarpPb &obj)
...
//________________________________________________________________________
AliXiStarpPb &AliXiStarpPb::operator=(const AliXiStarpPb &obj)
{
    // Assignment operator
    if (this == &obj)
        return *this;
...
void AliXiStarpPb::XiStarInit()
{
    ...
    cout<<"AliXiStarpPb XiStarInit() call"<<endl;
    ...
    fZvertexBins = 20;
    fMultBins = 11;// This must also be set in AliXiStarpPb.h
    ...
    fEC = new AliXiStarpPbEventCollection **[fZvertexBins];
    for(unsigned short i=0; i<fZvertexBins; i++){
        
        fEC[i] = new AliXiStarpPbEventCollection *[fMultBins];
        
        for(unsigned short j=0; j<fMultBins; j++){
            
            fEC[i][j] = new AliXiStarpPbEventCollection(fEventsToMix+1);
        }
    }
    
    fTempStruct = new AliXiStarpPbTrackStruct[kNbinsM*8];
    ...
    fCutValues[0][4] = 0.06;
    fCutValues[0][5] = 0.06;
    fCutValues[0][6] = 0.015;
    fCutValues[0][7] = 0.015;
    fCutValues[0][8] = 2.0;
    fCutValues[0][9] = 1.4;
    fCutValues[0][10] = 1.9;
    fCutValues[0][11] = 0.875;
    fCutValues[0][12] = 0.981;
    ...
    fCutValues[7][9] = 1.5;
    fCutValues[8][10] = 2.0;
    fCutValues[9][11] = 0.85;
    fCutValues[10][12] = 0.976;

    //systematic variation// tight
    fCutValues[11][0] = 80; fCutValues[11][1] = 80; fCutValues[11][2] = 80; fCutValues[11][3] = 80;// 80
    fCutValues[12][4] = 0.07;
    fCutValues[13][5] = 0.07;
    fCutValues[14][6] = 0.02;
    fCutValues[15][7] = 0.02;
    fCutValues[16][8] = 1.9;
    fCutValues[17][9] = 1.3;
    fCutValues[18][10] = 1.8;
    fCutValues[19][11] = 0.9;
    fCutValues[20][12] = 0.986;
...
//________________________________________________________________________
void AliXiStarpPb::UserCreateOutputObjects()
{
    XiStarInit();
   ...
   // fOutputList->Add(fPhiPtDist);
    ...
    TH1F *fQAXiYDistMC1 = new TH1F("fQAXiYDistMC1","fQAXiYDistMC1 in MC",400,-2,2);
    fQAXiYDistMC1->GetXaxis()->SetTitle("fQAXiYDistMC1");
    fOutputList->Add(fQAXiYDistMC1);
    ...
    TH1F *fQAXiStarYDistMC1 = new TH1F("fQAXiStarYDistMC1","fQAXiStarYDist in MC",400,-2,2);
    fQAXiStarYDistMC1->GetXaxis()->SetTitle("fQAXiStarYDistMC1");
    fOutputList->Add(fQAXiStarYDistMC1);
    ...
    TH1F *fQAXiYDistMCout = new TH1F("fQAXiYDistMCout","fQAXiYDistMC in MC output",400,-2,2);
    fQAXiYDistMCout->GetXaxis()->SetTitle("fQAXiYDistMCout");
    fOutputList->Add(fQAXiYDistMCout);
    ...
    TH1F *fQAXiStarYDistMCout = new TH1F("fQAXiStarYDistMCout","fQAXiYDistMC in MC output",400,-2,2);
    fQAXiStarYDistMCout->GetXaxis()->SetTitle("fQAXiStarYDistMCout");
    fOutputList->Add(fQAXiStarYDistMCout);


    // Histograms for model prediction
    TH1F *fNPion = new TH1F("fNPion","fNPion in MC output",200,50*0.001,250*0.001);
    fNPion->GetXaxis()->SetTitle("fNPion");
    fOutputList->Add(fNPion);
    
    TH1F *fNLambda = new TH1F("fNLambda","fNLambda in MC output",200,1000*0.001,1200*0.001);
    fNLambda->GetXaxis()->SetTitle("fNLambda");
    fOutputList->Add(fNLambda);
    
    TH1F *fNXi = new TH1F("fNXi","fNXi in MC output",200,1220*0.001,1420*0.001);
    fNXi->GetXaxis()->SetTitle("fNXi");
    fOutputList->Add(fNXi);
    
    TH1F *fNSigmaPlus = new TH1F("fNSigmaPlus","fNSigmaPlus in MC output",200,1285*0.001,1485*0.001);
    fNSigmaPlus->GetXaxis()->SetTitle("fNSigmaPlus");
    fOutputList->Add(fNSigmaPlus);
    
    TH1F *fNSigmaMinus = new TH1F("fNSigmaMinus","fNSigmaMinus in MC output",200,1285*0.001,1485*0.001);
    fNSigmaMinus->GetXaxis()->SetTitle("fNSigmaMinus");
    fOutputList->Add(fNSigmaMinus);
    
    TH1F *fNXiStar = new TH1F("fNXiStar","fNXiStar in MC output",200,1430*0.001,1630*0.001);
    fNXiStar->GetXaxis()->SetTitle("fNXiStar");
    fOutputList->Add(fNXiStar);
    ...
    for(Int_t cv=0; cv<kNCutVariations; cv++){
        ...
        if(cv==0){
            ...
            *nameXi += cv;
            *nameXibar += cv;
            CutVar[cv].fXi = new TH3F(nameXi->Data(),"Invariant Mass Distribution", 100,0,10, 10,0,100, 200,1.2,1.4);
            fOutputList->Add(CutVar[cv].fXi);
            CutVar[cv].fXibar = new TH3F(nameXibar->Data(),"Invariant Mass Distribution", 100,0,10, 10,0,100, 200,1.2,1.4);
            ...
            *nameMCrecXi += cv;
            *nameMCrecXibar += cv;
            CutVar[cv].fMCrecXi = new TH3F(nameMCrecXi->Data(),"Invariant Mass Distribution", 100,0,10, 10,0,100, 200,1.2,1.4);
           CutVar[cv].fMCrecXibar = new TH3F(nameMCrecXibar->Data(),"Invariant Mass Distribution", 100,0,10, 10,0,100, 200,1.2,1.4);
        ...
        *nameXiPlusPiMinusbkg += cv;
        CutVar[cv].fXiMinusPiPlus  = new TH3F(nameXiMinusPiPlus->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 200,1.4,1.6);
        CutVar[cv].fXiMinusPiMinus = new TH3F(nameXiMinusPiMinus->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 200,1.4,1.6);
        CutVar[cv].fXiPlusPiPlus   = new TH3F(nameXiPlusPiPlus->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 200,1.4,1.6);
        CutVar[cv].fXiPlusPiMinus  = new TH3F(nameXiPlusPiMinus->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 200,1.4,1.6);
        CutVar[cv].fXiMinusPiPlusbkg  = new TH3F(nameXiMinusPiPlusbkg->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 200,1.4,1.6);
        CutVar[cv].fXiMinusPiMinusbkg = new TH3F(nameXiMinusPiMinusbkg->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 200,1.4,1.6);
        CutVar[cv].fXiPlusPiPlusbkg   = new TH3F(nameXiPlusPiPlusbkg->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 200,1.4,1.6);
        CutVar[cv].fXiPlusPiMinusbkg  = new TH3F(nameXiPlusPiMinusbkg->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 200,1.4,1.6);
        ...
        *nameMCrecXiPlusPiMinus += cv;
        CutVar[cv].fMCrecXiMinusPiPlus  = new TH3F(nameMCrecXiMinusPiPlus->Data(),"Invariant Mass Distribution", 100,0,10, 10,0,100, 200,1.4,1.6);
        CutVar[cv].fMCrecXiPlusPiMinus  = new TH3F(nameMCrecXiPlusPiMinus->Data(),"Invariant Mass Distribution", 100,0,10, 10,0,100, 200,1.4,1.6);
    ...
    TH3F *fMCinputTotalXiStar1 = new TH3F("fMCinputTotalXiStar1","Invariant Mass Distribution", 100,0,10, 10,0,100, 200,1.4,1.6);
    TH3F *fMCinputTotalXiStarbar1 = new TH3F("fMCinputTotalXiStarbar1","Invariant Mass Distribution", 100,0,10, 10,0,100, 200,1.4,1.6);
    fOutputList->Add(fMCinputTotalXiStar1);
    fOutputList->Add(fMCinputTotalXiStarbar1);
    
    TH3F *fMCinputTotalXi1 = new TH3F("fMCinputTotalXi1","Invariant Mass Distribution", 100,0,10, 10,0,100, 200,1.2,1.4);
    TH3F *fMCinputTotalXibar1 = new TH3F("fMCinputTotalXibar1","Invariant Mass Distribution", 100,0,10, 10,0,100, 200,1.2,1.4);
    fOutputList->Add(fMCinputTotalXi1);
    fOutputList->Add(fMCinputTotalXibar1);
    
    //
    
    TH3F *fMCinputTotalXiStar3 = new TH3F("fMCinputTotalXiStar3","Invariant Mass Distribution", 100,0,10, 10,0,100, 200,1.4,1.6);
    TH3F *fMCinputTotalXiStarbar3 = new TH3F("fMCinputTotalXiStarbar3","Invariant Mass Distribution", 100,0,10, 10,0,100, 200,1.4,1.6);
    fOutputList->Add(fMCinputTotalXiStar3);
    fOutputList->Add(fMCinputTotalXiStarbar3);
    
    TH3F *fMCinputTotalXi3 = new TH3F("fMCinputTotalXi3","Invariant Mass Distribution", 100,0,10, 10,0,100, 200,1.2,1.4);
    TH3F *fMCinputTotalXibar3 = new TH3F("fMCinputTotalXibar3","Invariant Mass Distribution", 100,0,10, 10,0,100, 200,1.2,1.4);
...
//________________________________________________________________________
void AliXiStarpPb::Exec(Option_t *)
{
       ...
       fEventNumber =
        ...
        if(fMCcase){
            ...
            for (Int_t it = 0; it < mcstack->GetNprimary(); it++) {
                ...
                if(MCxiStarY<-0.465 ||MCxiStarY>0.035) continue; // here selection of rapidity for pPb analysis
                ((TH1F*)fOutputList->FindObject("fQAXiStarYDistMC"))->Fill(MCxiStarY);
                // Xi
                if(mcInputTrack->GetPdgCode() == +kXiCode) {((TH3F*)fOutputList->FindObject("fMCinputTotalXi1"))->Fill(mcInputTrack->Pt(), centralityV0A, mcInputTrack->GetCalcMass());
                               }
                
                if(mcInputTrack->GetPdgCode() == -kXiCode) {((TH3F*)fOutputList->FindObject("fMCinputTotalXibar1"))->Fill(mcInputTrack->Pt(), centralityV0A, mcInputTrack->GetCalcMass());
                              }
                // XiStar
                if(mcInputTrack->GetPdgCode() == +kXiStarCode) {((TH3F*)fOutputList->FindObject("fMCinputTotalXiStar1"))->Fill(mcInputTrack->Pt(), centralityV0A, mcInputTrack->GetCalcMass());
                                  }
                if(mcInputTrack->GetPdgCode() == -kXiStarCode) {((TH3F*)fOutputList->FindObject("fMCinputTotalXiStarbar1"))->Fill(mcInputTrack->Pt(),centralityV0A, mcInputTrack->GetCalcMass());
 ...
 /* TPC OFF */           if(nsigpi>nSigTPCPID) continue; // last update // 20140715 // TPC
    ...
    if(fMCcase){
        ...
        for (Int_t it = 0; it < mcstack->GetNprimary(); it++) {
            TParticle *mcModelPrediction = (TParticle*)mcstack->Particle(it);
            if (!mcModelPrediction) {
                Error("UserExec", "Could not receive track %d", it);
                continue;
            }
            if(abs(mcModelPrediction->GetPdgCode())==kPionCode) ((TH1F*)fOutputList->FindObject("fNPion"))->Fill(mcModelPrediction->GetCalcMass());
            if(abs(mcModelPrediction->GetPdgCode())==kLambdaCode) ((TH1F*)fOutputList->FindObject("fNLambda"))->Fill(mcModelPrediction->GetCalcMass());
            if(abs(mcModelPrediction->GetPdgCode())==kXiCode) ((TH1F*)fOutputList->FindObject("fNXi"))->Fill(mcModelPrediction->GetCalcMass());
            if(abs(mcModelPrediction->GetPdgCode())==kSigmaStarPCode) ((TH1F*)fOutputList->FindObject("fNSigmaPlus"))->Fill(mcModelPrediction->GetCalcMass());
            if(abs(mcModelPrediction->GetPdgCode())==kSigmaStarMCode) ((TH1F*)fOutputList->FindObject("fNSigmaMinus"))->Fill(mcModelPrediction->GetCalcMass());
            if(abs(mcModelPrediction->GetPdgCode())==kXiStarCode) ((TH1F*)fOutputList->FindObject("fNXiStar"))->Fill(mcModelPrediction->GetCalcMass());
        }
    ...
    if(fMCcase){// get Input MC information for ESD case
        ...
        for (Int_t it = 0; it < mcstack->GetNprimary(); it++) {
            ...
            if(mcInputTrackXi->Y()>-0.465 && mcInputTrackXi->Y()<0.035){
            ((TH1F*)fOutputList->FindObject("fQAXiYDistMC1"))->Fill(mcInputTrackXi->Y());
            }
            
            
            if(mcInputTrackXi->GetPdgCode() == +kXiCode) {
                ((TH3F*)fOutputList->FindObject("fMCinputTotalXi3"))->Fill(mcInputTrackXi->Pt(), centralityV0A, mcInputTrackXi->GetCalcMass());
            }
            else {((TH3F*)fOutputList->FindObject("fMCinputTotalXibar3"))->Fill(mcInputTrackXi->Pt(), centralityV0A, mcInputTrackXi->GetCalcMass());
        ...
        for (Int_t it = 0; it < mcstack->GetNprimary(); it++) {
            ...
            if(mcInputTrackXiStar->Y()<-0.465 ||mcInputTrackXiStar->Y()>0.035) continue; // here selection of rapidity for pPb analysis
            ((TH1F*)fOutputList->FindObject("fQAXiStarYDistMC1"))->Fill(mcInputTrackXiStar->Y());
            
            

            if(mcInputTrackXiStar->GetPdgCode() == +kXiStarCode) {((TH3F*)fOutputList->FindObject("fMCinputTotalXiStar3"))->Fill(mcInputTrackXiStar->Pt(), centralityV0A, mcInputTrackXiStar->GetCalcMass());
            }
            else {((TH3F*)fOutputList->FindObject("fMCinputTotalXiStarbar3"))->Fill(mcInputTrackXiStar->Pt(), centralityV0A, mcInputTrackXiStar->GetCalcMass());
 ...
 /* TPC PID OFF */      if(fTPCNSigPion2<-nSigTPCPID||fTPCNSigPion2>nSigTPCPID) continue; // PID for 2nd pion
   ...
   //     ((TH1F*)fOutputList->FindObject("fRXY_Xi"))->Fill(fDecayParameters[12]);
       ...
       // if(StandardXi)((TH1F*)fOutputList->FindObject("fCutEvents"))->Fill(12,1);
        ...
        if(StandardXi){
            if(xiCharge == -1) {
                CutVar[0].fXi->Fill(xiPt, centralityV0A, xiMass);
                ((TH1F*)fOutputList->FindObject("hXiInvMass"))->Fill(xiMass);
            }
            else {
                CutVar[0].fXibar->Fill(xiPt, centralityV0A, xiMass);
                ((TH1F*)fOutputList->FindObject("hXiInvMass"))->Fill(xiMass);
            }
        }
                                ...
                                if(MCLamesd->GetMother(0) == MCXiD2esd->GetMother(0)){
                                    ...
                                    if(abs(MCXiesd->GetPdgCode())==kXiCode) {
                                        ...
                                        if(StandardXi){
                                            ...
                                            if(xiY>-0.465 && xiY < 0.035) {
                                                ((TH1F*)fOutputList->FindObject("fQAXiYDistMCout"))->Fill(xiY);}
                                            if(Xicandidate->Charge() == -1){
                                                
                                                CutVar[0].fMCrecXi->Fill(xiPt, centralityV0A, xiMass);
                                            }else{
                                                CutVar[0].fMCrecXibar->Fill(xiPt, centralityV0A, xiMass);
                                            }
        ...
        for(Int_t EN=0; EN<fEventsToMix+1; EN++){// Event buffer loop
            ...
            for(Int_t l=0; l<(fEvt+EN)->fNTracks; l++){// Present(EN=0) and Past(EN from 1 to fEventsToMix) event track loop
                ...
                if(xiStarY<-0.465 ||xiStarY>0.035) continue; // here selection of rapidity for pPb analysis
                ...
                for(int cv=0; cv<kNCutVariations; cv++){
                    ...
                    if(EN==0){
                        if(fXiTrack->Charge() == -1 &&  fESDTrack4->Charge() == -1) CutVar[cv].fXiMinusPiMinus->Fill(xiStarPt, centralityV0A, xiStarMass);
                        else if(fXiTrack->Charge() == -1 &&  fESDTrack4->Charge() == +1) {
                            CutVar[cv].fXiMinusPiPlus->Fill(xiStarPt, centralityV0A, xiStarMass);
                        }
                        else if(fXiTrack->Charge() == +1 &&  fESDTrack4->Charge() == -1) {
                            CutVar[cv].fXiPlusPiMinus->Fill(xiStarPt, centralityV0A, xiStarMass);
                        }
                        else CutVar[cv].fXiPlusPiPlus->Fill(xiStarPt, centralityV0A, xiStarMass);
                    }else {
                        if(fXiTrack->Charge() == -1 &&  fESDTrack4->Charge() == -1) CutVar[cv].fXiMinusPiMinusbkg->Fill(xiStarPt, centralityV0A, xiStarMass);
                        else if(fXiTrack->Charge() == -1 &&  fESDTrack4->Charge() == +1) CutVar[cv].fXiMinusPiPlusbkg->Fill(xiStarPt, centralityV0A, xiStarMass);
                        else if(fXiTrack->Charge() == +1 &&  fESDTrack4->Charge() == -1) CutVar[cv].fXiPlusPiMinusbkg->Fill(xiStarPt, centralityV0A, xiStarMass);
                        else CutVar[cv].fXiPlusPiPlusbkg->Fill(xiStarPt, centralityV0A, xiStarMass);
                    }
                        ...
                        if(abs(MCXiStarD2esd->GetPdgCode())==kPionCode){
                            if(MCXiesd->GetMother(0) == MCXiStarD2esd->GetMother(0)){
                                ...
                                if(abs(MCXiStaresd->GetPdgCode())==kXiStarCode) {
                                    ...
                                    if(xiStarY<-0.465 ||xiStarY>0.035) continue;
                                    ((TH1F*)fOutputList->FindObject("fQAXiStarYDistMCout"))->Fill(xiStarY);

                                    
                                    if(fXiTrack->Charge() == -1 &&  fESDTrack4->Charge() == +1) CutVar[cv].fMCrecXiMinusPiPlus->Fill(xiStarPt, centralityV0A, xiStarMass);
                                    if(fXiTrack->Charge() == +1 &&  fESDTrack4->Charge() == -1) CutVar[cv].fMCrecXiPlusPiMinus->Fill(xiStarPt, centralityV0A, xiStarMass);
...
//________________________________________________________________________
void AliXiStarpPb::Terminate(Option_t *)
{
    cout<<"Done"<<endl;
}
//________________________________________________________________________
/*
Double_t AliXiStarpPb::LinearPropagateToDCA(AliESDtrack *v, AliESDtrack *t, Double_t b) {// Adapted from AliCascadeVertexer.cxx
    //--------------------------------------------------------------------
...
//________________________________________________________________________
Double_t AliXiStarpPb::Det(Double_t a00, Double_t a01, Double_t a10, Double_t a11) const {// Taken from AliCascadeVertexer
    //--------------------------------------------------------------------
...
//________________________________________________________________________
Double_t AliXiStarpPb::Det(Double_t a00,Double_t a01,Double_t a02,
