...
//    plugin->SetROOTVersion("v5-34-08-6"); // was used for LHC13b for real production
//    plugin->SetAliROOTVersion("vAN-20141202");
    
    
    plugin->SetAliROOTVersion("v5-06-39");
    plugin->SetAliPhysicsVersion("v5-06-39-01");
...
//    if(!MCcase) plugin->SetGridWorkingDir("2016_LHC13b_ChangeCPA");
    if(!MCcase) plugin->SetGridWorkingDir("2016_LHC13c_ChangeCPA");
    ...
    plugin->SetDefaultOutputs(kTRUE); //20150420 //kTRUE
  //  plugin->SetOutputFiles("MyOutput.root"); //20150420
    // No need define the files to be archived. Note that this is handled automatically by the plugin.
  //  plugin->SetOutputArchive("log_archive.zip:stdout,stderr"); //20150420
...
Int_t AddGoodRuns(AliAnalysisAlien* plugin,TString lhcPeriod,bool MCcase=kFALSE) {
    ...
    if(lhcPeriod=="LHC13b") {
        nruns=12; //
       Int_t runlist[12]={195344, 195346, 195351, 195389, 195390, 195391, 195478, 195479, 195480, 195481, 195482, 195483};
    
    if(lhcPeriod=="LHC13c") {
        nruns=14; //14
        Int_t runlist[2]={195677, 195675, 195673, 195644, 195635, 195633, 195596, 195593, 195592, 195568, 195567, 195566, 195531, 195529}; //14
