...
Anal(Int_t split=-1){
   ...
   // Float_t fTreeVariableEventCounter3;
    ...
    TH1F *XiRap = new TH1F("XiRap","Xi rapidity distribution before mass window",400,-2,2);
    
    TH2F *hEventCount = new TH2F("hEventCount","Event count for each centrality",100,0,100,1000,0,1000);
    TH2F *hEventCount1 = new TH2F("hEventCount1","Event count for each centrality",100,0,100,1000,0,1000);
    TH2F *hEventCount2 = new TH2F("hEventCount2","Event count for each centrality",100,0,100,1000,0,1000);
    TH2F *hEventCount3 = new TH2F("hEventCount3","Event count for each centrality",100,0,100,1000,0,1000);

    
 //test   TH1F *XiRap_a = new TH1F("XiRap_a","Xi rapidity distribution",400,-2,2);
    ...
    for(int ii=0; ii<8; ii++){
        ...
        for(int jj=0; jj<14; jj++){
            ...
            fXiStarY[ii][jj] = new TH1D(Form("fXiStarY_%d_%02d",ii,jj),Form("XiStar Mass vs. Rapidity_%d_%02d",ii,jj),400,-2,2);
 ...
 */
    ...
    TH1F *XiStarRap = new TH1F("XiStarRap","XiStar rapidity distribution",40,-2,2);

    
    Double_t px1,py1,pz1, px2,py2,pz2;
    Double_t p1sq,p2sq,e1,e2,angle;
    Double_t  fTrueMassPr=.93827, fTrueMassPi=.13957, fTrueMassK=.493677, fTrueMassLam=1.11568, fTrueMassXi=1.32171, fTrueMassXiStar=1.5318, fMassWindow=0.007;
 ...
 //   fTreePi->SetBranchAddress("fTreeVariableEventCounter3",&fTreeVariableEventCounter3);
   ...
   */ 
    ...
    for (Long64_t ev=0; ev<fTreeEvent->GetEntries(); ev++){
        ...
        if(fTreeVariableCentrality>=0.&&fTreeVariableCentrality<20.)hEventCount->Fill(fTreeVariableCentrality,fTreeVariableEventNumberInFile);
        if(fTreeVariableCentrality>=20.&&fTreeVariableCentrality<40.)hEventCount1->Fill(fTreeVariableCentrality,fTreeVariableEventNumberInFile);
        if(fTreeVariableCentrality>=40.&&fTreeVariableCentrality<60.)hEventCount2->Fill(fTreeVariableCentrality,fTreeVariableEventNumberInFile);
        if(fTreeVariableCentrality>=60.&&fTreeVariableCentrality<100.)hEventCount3->Fill(fTreeVariableCentrality,fTreeVariableEventNumberInFile);
        ...
        //test    XiRap_a->Fill(fTreeVariableY);
            ...
            for(Long64_t l=0; l<fTreeVariablemyTracks; l++){// Present(EN=0) and Past(EN from 1 to fEventsToMix) event track loop
                ...
                xiStarY = .5*log( ((e1+e2) + xiStarP[2])/((e1+e2) - xiStarP[2] + 1.e-13));
                
                 // Dhevan xiStarY = .5*log( ((e1+e2) + xiStarP[2])/((e1+e2) - xiStarP[2]));
                //xiStarE = e1 + e2;
               ...
               // if(fabs(xiStarMass-fTrueMassXiStar) > fMassWindow) continue;
                 XiStarRap->Fill(xiStarY);
                ...
                for(int cv=0; cv<kNCutVariations; cv++){
                    ...
                    //*** Rapidity info *** //
                    double minYlab = -0.965;
                    double zeroYlab = -0.465;
                    double maxYlab = 0.035;
                    // rapidity  |y| < 0.5 // same as pp (PbPb)
                    //if(fTreeVariableY > -0.5 && fTreeVariableY < 0.5 )
                    // rapidity 0 < y_cm < 0.5 // for pPb
                    
                    //if(fTreeVariableY > zeroYlab  && fTreeVariableY < maxYlab )
                    // rapidity -0.5 < y_cm < 0 // for pPb
                        
                    //if(fTreeVariableY > minYlab  && fTreeVariableY < zeroYlab )
                    // rapidity  |y_cm| < 0.3  // for pPb
                            
                    //if(fTreeVariableY > -0.765  && fTreeVariableY < -0.165 )
                                
                                
                    if(xiStarY < zeroYlab || xiStarY > maxYlab ) continue;
                    if(EN==0){
                        if (!(cv == 0)) continue;
    ...
    TFile *fout = TFile::Open(ss+"/Rap0toplus5_EventCount_2015_feb26.root","RECREATE");
  //  TFile *fout = TFile::Open(ss+"/25Sep_rap_pp.root","RECREATE");
