AliAnalysisGrid* CreateAlienHandler(bool MCcase)
{
    ...
    if(!MCcase) plugin->SetGridWorkingDir("LHC13c_Data");
  //  else plugin->SetGridWorkingDir("lhc13b2_efix_p2_Cent_Nov29");
  //  else plugin->SetGridWorkingDir("lhc13b2_efix_p2_Cent_Nov30");
    else plugin->SetGridWorkingDir("Xi_LHC13b2_efix_p1_march19"); // before merging
...
Int_t AddGoodRuns(AliAnalysisAlien* plugin,TString lhcPeriod,bool MCcase=kFALSE) {
    ...
    if(lhcPeriod=="LHC13c") {
        nruns=24; // 150 runs
        Int_t runlist[24]={195344, 195346, 195351, 195389, 195390, 195391, 195478, 195479, 195480, 195481, 195482, 195483, 195529, 195531, 195566, 195567, 195568, 195592,  195596, 195633, 195635,  195673, 195675, 195677}; //195593,
