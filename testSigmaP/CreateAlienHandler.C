AliAnalysisGrid* CreateAlienHandler(bool MCcase)
{
  ...
  //   plugin->SetAliPhysicsVersion("vAN-20160616-1");
    ...
    if(!MCcase) plugin->SetGridWorkingDir("zzzzTestPi0");
    //if(!MCcase) plugin->SetGridWorkingDir("LHC10h_NewStatTest_June4_Signal");

  //  else plugin->SetGridWorkingDir("LHC11h_MC_kCentral_LHC14a1a_June6");
   // else plugin->SetGridWorkingDir("LHC11h_MC_kSemiCentral_LHC14a1b_June6");
    //else plugin->SetGridWorkingDir("LHC11h_MC_kMB_LHC14a1c_June6");

     else plugin->SetGridWorkingDir("LHC11h_MC_kCent_LHC14a1a_JUNE28");
    ...
    plugin->SetAnalysisSource("AliSigmaPEventCollection.cxx AliSigmaP.cxx");
    // Declare all libraries (other than the default ones for the framework. These will be
    // loaded by the generated analysis macro. Add all extra files (task .cxx/.h) here.
    plugin->SetAdditionalLibs("AliSigmaPEventCollection.h AliSigmaPEventCollection.cxx AliSigmaP.h AliSigmaP.cxx");
    ...
    plugin->SetSplitMaxInputFileNumber(10);// // TURN ON for data//if(!MCcase)
    ...
    if(lhcPeriod=="LHC10d") {
        ...
        if(MCcase){
            ...
            if(SetRunNumber==2){
                ...
                Int_t runlist[5]={ 170315, 170387, 170388, 170572, 170593};
            }
