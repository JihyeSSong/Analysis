...
// #include "AliVEventHandler.h" //march26 // remove April 27
...

{
   ...
   /*     CutVar[cv].sXiMinusPiPlus=0x0;
        CutVar[cv].sXiMinusPiMinus=0x0;
        CutVar[cv].sXiPlusPiPlus=0x0;
        CutVar[cv].sXiPlusPiMinus=0x0;
        //
        CutVar[cv].sXiMinusPiPlusbkg=0x0;
        CutVar[cv].sXiMinusPiMinusbkg=0x0;
        CutVar[cv].sXiPlusPiPlusbkg=0x0;
        CutVar[cv].sXiPlusPiMinusbkg=0x0;
        //
        CutVar[cv].sMCrecXiMinusPiPlus=0x0;
        CutVar[cv].sMCrecXiPlusPiMinus=0x0;
    ...
    {
    // ROOT IO constructor, don’t allocate memory here! // add May 10
    }
}
//________________________________________________________________________
AliSigmaP::AliSigmaP(const char *name, Bool_t AODdecision,  Bool_t MCdecision, Int_t CutListOption)
...
//________________________________________________________________________
AliSigmaP::AliSigmaP(const AliSigmaP &obj)
: AliAnalysisTaskSE(obj.fname),
fname(obj.fname),
fESD(obj.fESD),
fOutputList(obj.fOutputList),
fOutputListL(obj.fOutputListL),
fOutputListT(obj.fOutputListT),

//fOutputListSparse(obj.fOutputListSparse),
...
//________________________________________________________________________
AliSigmaP &AliSigmaP::operator=(const AliSigmaP &obj)
{
    // Assignment operator
    if (this == &obj)
        return *this;
...
AliSigmaP::~AliSigmaP()
{
    ...
    if(fOutputListL) delete fOutputListL;
    if(fOutputListT) delete fOutputListT;

//    if(fOutputListSparse) delete fOutputListSparse;
 ...
 /*       if(CutVar[cv].sXiMinusPiPlus) delete CutVar[cv].sXiMinusPiPlus;
        if(CutVar[cv].sXiMinusPiMinus) delete CutVar[cv].sXiMinusPiMinus;
        if(CutVar[cv].sXiPlusPiPlus) delete CutVar[cv].sXiPlusPiPlus;
        if(CutVar[cv].sXiPlusPiMinus) delete CutVar[cv].sXiPlusPiMinus;
        //
        if(CutVar[cv].sXiMinusPiPlusbkg) delete CutVar[cv].sXiMinusPiPlusbkg;
        if(CutVar[cv].sXiMinusPiMinusbkg) delete CutVar[cv].sXiMinusPiMinusbkg;
        if(CutVar[cv].sXiPlusPiPlusbkg) delete CutVar[cv].sXiPlusPiPlusbkg;
        if(CutVar[cv].sXiPlusPiMinusbkg) delete CutVar[cv].sXiPlusPiMinusbkg;
        //
        if(CutVar[cv].sMCrecXiMinusPiPlus) delete CutVar[cv].sMCrecXiMinusPiPlus;
        if(CutVar[cv].sMCrecXiPlusPiMinus) delete CutVar[cv].sMCrecXiPlusPiMinus;
  */
...
//________________________________________________________________________
void AliSigmaP::XiStarInit()
{
    //
    //Inits cuts and analysis settings
    //
    
    cout<<"AliSigmaP XiStarInit() call"<<endl;
 ...
 //inpPb   fMultBins = 11;// This must also be set in AliSigmaP.h
    
    fZvertexBins = 10;
  //  fMultBins = 2;// for central collisions
  //  fMultBins = 4;// for semi - central collisions
    fMultBins = 10;// for all collisions

    
   fEventsToMix = 0; // original 40 in pp, 20 in p-Pb, 5 in Pb-Pb
  //  else fEventsToMix = 0; // original 40 jisong
   ...
   // fMultLimits[0]=0, fMultLimits[1]=5000, fMultLimits[2]=10000, fMultLimits[3]=15000, fMultLimits[4]=20000; // for semi-central collisions

   // fMultLimits[0]=0, fMultLimits[1]=150, fMultLimits[2]=300, fMultLimits[3]=450, fMultLimits[4]=600, fMultLimits[5]=750, fMultLimits[6]=900, fMultLimits[7]=1050, fMultLimits[8]=1200, fMultLimits[9]=1300, fMultLimits[10]=1400;
    
   // fMultLimits[0]=0, fMultLimits[1]=1250, fMultLimits[2]=2500, fMultLimits[3]=3750, fMultLimits[4]=5000, fMultLimits[5]=6250, fMultLimits[6]=7500, fMultLimits[7]=8750, fMultLimits[8]=10000, fMultLimits[9]=13000, fMultLimits[10]=20000; // was used at SPARSE
    
    fMultLimits[0]=0, fMultLimits[1]=500, fMultLimits[2]=1000, fMultLimits[3]=1500, fMultLimits[4]=2000, fMultLimits[5]=2500, fMultLimits[6]=3000, fMultLimits[7]=3500, fMultLimits[8]=4000, fMultLimits[9]=5000, fMultLimits[10]=7000; // estimated from fMultDist5 (June 21. 2016)

    
    
    fEC = new AliSigmaPEventCollection **[fZvertexBins];
    for(unsigned short i=0; i<fZvertexBins; i++){
        
        fEC[i] = new AliSigmaPEventCollection *[fMultBins];
        
        for(unsigned short j=0; j<fMultBins; j++){
            
            fEC[i][j] = new AliSigmaPEventCollection(fEventsToMix+1);
        }
    }
    
    fTempStruct = new AliSigmaPTrackStruct[1000]; //1000 was best but wired EM bg distribution comes from this ?!//20000 for BG
...
//________________________________________________________________________
void AliSigmaP::UserCreateOutputObjects()
{
    XiStarInit();
    ...
    TH3F *fVertexDistXYZQA = new TH3F("fVertexDistXYZQA","Vertex Distribution",20,-1,1, 20,-1,1, 60,-30,30);
    fVertexDistXYZQA->GetXaxis()->SetTitle("X Vertex (cm)");
    fVertexDistXYZQA->GetYaxis()->SetTitle("Y Vertex (cm)");
    fVertexDistXYZQA->GetZaxis()->SetTitle("Z Vertex (cm)");
    fOutputList->Add(fVertexDistXYZQA);
    
    
    TH1F *fVertexDistX = new TH1F("fVertexDistX","Vertex Distribution",200,-0.2,0.2);
    fOutputList->Add(fVertexDistX);
    TH1F *fVertexDistY = new TH1F("fVertexDistY","Vertex Distribution",200,-0.4,0.4);
    fOutputList->Add(fVertexDistY);
    TH1F *fVertexDistZ = new TH1F("fVertexDistZ","Vertex Distribution",600,-30,30);
    fOutputList->Add(fVertexDistZ);
 ...
 */
   ...
   //******* histos for pi0 *********//
     TH2F *hArmenterosBefore=new TH2F("hArmenterosBefore","Armenteros_before",200,-1,1,1000,0,1.);
    fOutputList->Add(hArmenterosBefore);
    
    TH2F *hArmenterosAfter=new TH2F("hArmenterosAfter","Armenteros_after",200,-1,1,1000,0,1.);
    fOutputList->Add(hArmenterosAfter);
    
    TH1F *hPi0Mass=new TH1F("hPi0Mass","hPi0 Mass",400,0,0.4);
    fOutputList->Add(hPi0Mass);
    
    TH1F *hPi0Mass1=new TH1F("hPi0Mass1","hPi01 Mass",400,0,0.4);
    fOutputList->Add(hPi0Mass1);
    ...
    fOutputListL = new TList();
    fOutputListL->SetOwner();

    fOutputListT = new TList();
    fOutputListT->SetOwner();
     ...
     for(Int_t cv=0; cv<kNCutVariations; cv++){
        ...
        *nameXiPlusPiMinusbkg += cv;
        CutVar[cv].fXiMinusPiPlus  = new TH3F(nameXiMinusPiPlus->Data(),"Invariant Mass Distribution", 100,0,10,100,0,100, 200,1.4,1.8);
        CutVar[cv].fXiMinusPiMinus = new TH3F(nameXiMinusPiMinus->Data(),"Invariant Mass Distribution", 100,0,10,100,0,100, 200,1.4,1.8);
        CutVar[cv].fXiPlusPiPlus   = new TH3F(nameXiPlusPiPlus->Data(),"Invariant Mass Distribution", 100,0,10,100,0,100, 200,1.4,1.8);
        CutVar[cv].fXiPlusPiMinus  = new TH3F(nameXiPlusPiMinus->Data(),"Invariant Mass Distribution", 100,0,10,100,0,100, 200,1.4,1.8);
        CutVar[cv].fXiMinusPiPlusbkg  = new TH3F(nameXiMinusPiPlusbkg->Data(),"Invariant Mass Distribution", 100,0,10,100,0,100, 200,1.4,1.8);
        CutVar[cv].fXiMinusPiMinusbkg = new TH3F(nameXiMinusPiMinusbkg->Data(),"Invariant Mass Distribution", 100,0,10,100,0,100, 200,1.4,1.8);
        CutVar[cv].fXiPlusPiPlusbkg   = new TH3F(nameXiPlusPiPlusbkg->Data(),"Invariant Mass Distribution", 100,0,10,100,0,100, 200,1.4,1.8);
        CutVar[cv].fXiPlusPiMinusbkg  = new TH3F(nameXiPlusPiMinusbkg->Data(),"Invariant Mass Distribution",100,0,10,100,0,100, 200,1.4,1.8);
        ...
        *nameMCrecXiPlusPiMinus += cv;
        CutVar[cv].fMCrecXiMinusPiPlus  = new TH3F(nameMCrecXiMinusPiPlus->Data(),"Invariant Mass Distribution", 100,0,10,100,0,100, 200,1.4,1.8);
        CutVar[cv].fMCrecXiPlusPiMinus  = new TH3F(nameMCrecXiPlusPiMinus->Data(),"Invariant Mass Distribution", 100,0,10,100,0,100, 200,1.4,1.8);
        fOutputList->Add(CutVar[cv].fMCrecXiMinusPiPlus);
        fOutputList->Add(CutVar[cv].fMCrecXiPlusPiMinus);
        //
        }
         if(cv>0 && cv<11){ TString *nameXiMinusPiPlus = new TString("fXiMinusPiPlus_");
             TString *nameXiMinusPiMinus = new TString("fXiMinusPiMinus_");
             TString *nameXiPlusPiPlus = new TString("fXiPlusPiPlus_");
             TString *nameXiPlusPiMinus = new TString("fXiPlusPiMinus_");
             TString *nameXiMinusPiPlusbkg = new TString("fXiMinusPiPlusbkg_");
             ...
             *nameXiPlusPiMinusbkg += cv;
             ...
             fOutputListL->Add(CutVar[cv].fXiMinusPiPlus);
             fOutputListL->Add(CutVar[cv].fXiMinusPiMinus);
             fOutputListL->Add(CutVar[cv].fXiPlusPiPlus);
             fOutputListL->Add(CutVar[cv].fXiPlusPiMinus);
             fOutputListL->Add(CutVar[cv].fXiMinusPiPlusbkg);
             fOutputListL->Add(CutVar[cv].fXiMinusPiMinusbkg);
             fOutputListL->Add(CutVar[cv].fXiPlusPiPlusbkg);
             fOutputListL->Add(CutVar[cv].fXiPlusPiMinusbkg);
             //
             ...
             *nameMCrecXiPlusPiMinus += cv;
             CutVar[cv].fMCrecXiMinusPiPlus  = new TH3F(nameMCrecXiMinusPiPlus->Data(),"Invariant Mass Distribution", 100,0,10,100,0,100, 200,1.4,1.8);
             CutVar[cv].fMCrecXiPlusPiMinus  = new TH3F(nameMCrecXiPlusPiMinus->Data(),"Invariant Mass Distribution", 100,0,10,100,0,100, 200,1.4,1.8);
             fOutputListL->Add(CutVar[cv].fMCrecXiMinusPiPlus);
             fOutputListL->Add(CutVar[cv].fMCrecXiPlusPiMinus);
}
         if(cv>10 && cv<21){ TString *nameXiMinusPiPlus = new TString("fXiMinusPiPlus_");
             TString *nameXiMinusPiMinus = new TString("fXiMinusPiMinus_");
             TString *nameXiPlusPiPlus = new TString("fXiPlusPiPlus_");
             TString *nameXiPlusPiMinus = new TString("fXiPlusPiMinus_");
             TString *nameXiMinusPiPlusbkg = new TString("fXiMinusPiPlusbkg_");
             ...
             *nameXiPlusPiMinusbkg += cv;
             ...
             fOutputListT->Add(CutVar[cv].fXiMinusPiPlus);
             fOutputListT->Add(CutVar[cv].fXiMinusPiMinus);
             fOutputListT->Add(CutVar[cv].fXiPlusPiPlus);
             fOutputListT->Add(CutVar[cv].fXiPlusPiMinus);
             fOutputListT->Add(CutVar[cv].fXiMinusPiPlusbkg);
             fOutputListT->Add(CutVar[cv].fXiMinusPiMinusbkg);
             fOutputListT->Add(CutVar[cv].fXiPlusPiPlusbkg);
             fOutputListT->Add(CutVar[cv].fXiPlusPiMinusbkg);
             //
             ...
             *nameMCrecXiPlusPiMinus += cv;
             CutVar[cv].fMCrecXiMinusPiPlus  = new TH3F(nameMCrecXiMinusPiPlus->Data(),"Invariant Mass Distribution", 100,0,10,100,0,100, 200,1.4,1.8);
             CutVar[cv].fMCrecXiPlusPiMinus  = new TH3F(nameMCrecXiPlusPiMinus->Data(),"Invariant Mass Distribution", 100,0,10,100,0,100, 200,1.4,1.8);
             fOutputListT->Add(CutVar[cv].fMCrecXiMinusPiPlus);
             fOutputListT->Add(CutVar[cv].fMCrecXiPlusPiMinus);
}
         // Add sparse variables
 /*        TString *snameXiMinusPiPlus = new TString("sXiMinusPiPlus_");
         TString *snameXiMinusPiMinus = new TString("sXiMinusPiMinus_");
         TString *snameXiPlusPiPlus = new TString("sXiPlusPiPlus_");
         TString *snameXiPlusPiMinus = new TString("sXiPlusPiMinus_");
         TString *snameXiMinusPiPlusbkg = new TString("sXiMinusPiPlusbkg_");
         TString *snameXiMinusPiMinusbkg = new TString("sXiMinusPiMinusbkg_");
         TString *snameXiPlusPiPlusbkg = new TString("sXiPlusPiPlusbkg_");
         TString *snameXiPlusPiMinusbkg = new TString("sXiPlusPiMinusbkg_");
         *snameXiMinusPiPlus += cv;
         *snameXiMinusPiMinus += cv;
         *snameXiPlusPiPlus += cv;
         *snameXiPlusPiMinus += cv;
         *snameXiMinusPiPlusbkg += cv;
         *snameXiMinusPiMinusbkg += cv;
         *snameXiPlusPiPlusbkg += cv;
         *snameXiPlusPiMinusbkg += cv;
         CutVar[cv].sXiMinusPiPlus  = new THnSparseF(snameXiMinusPiPlus->Data(),"Invariant Mass Distribution", nDim,nBins,xMin,xMax);
         CutVar[cv].sXiMinusPiMinus = new THnSparseF(snameXiMinusPiMinus->Data(),"Invariant Mass Distribution", nDim,nBins,xMin,xMax);
         CutVar[cv].sXiPlusPiPlus   = new THnSparseF(snameXiPlusPiPlus->Data(),"Invariant Mass Distribution", nDim,nBins,xMin,xMax);
         CutVar[cv].sXiPlusPiMinus  = new THnSparseF(snameXiPlusPiMinus->Data(),"Invariant Mass Distribution", nDim,nBins,xMin,xMax);
         CutVar[cv].sXiMinusPiPlusbkg  = new THnSparseF(snameXiMinusPiPlusbkg->Data(),"Invariant Mass Distribution",nDim,nBins,xMin,xMax);
         CutVar[cv].sXiMinusPiMinusbkg = new THnSparseF(snameXiMinusPiMinusbkg->Data(),"Invariant Mass Distribution",nDim,nBins,xMin,xMax);
         CutVar[cv].sXiPlusPiPlusbkg   = new THnSparseF(snameXiPlusPiPlusbkg->Data(),"Invariant Mass Distribution", nDim,nBins,xMin,xMax);
         CutVar[cv].sXiPlusPiMinusbkg  = new THnSparseF(snameXiPlusPiMinusbkg->Data(),"Invariant Mass Distribution",nDim,nBins,xMin,xMax);
         
         CutVar[cv].sXiMinusPiPlus->Sumw2();
         CutVar[cv].sXiMinusPiMinus->Sumw2();
         CutVar[cv].sXiPlusPiPlus->Sumw2();
         CutVar[cv].sXiPlusPiMinus->Sumw2();
         CutVar[cv].sXiMinusPiPlusbkg->Sumw2();
         CutVar[cv].sXiMinusPiMinusbkg->Sumw2();
         CutVar[cv].sXiPlusPiPlusbkg->Sumw2();
         CutVar[cv].sXiPlusPiMinusbkg->Sumw2();
         
         fOutputListSparse->Add(CutVar[cv].sXiMinusPiPlus);
         fOutputListSparse->Add(CutVar[cv].sXiMinusPiMinus);
         fOutputListSparse->Add(CutVar[cv].sXiPlusPiPlus);
         fOutputListSparse->Add(CutVar[cv].sXiPlusPiMinus);
         fOutputListSparse->Add(CutVar[cv].sXiMinusPiPlusbkg);
         fOutputListSparse->Add(CutVar[cv].sXiMinusPiMinusbkg);
         fOutputListSparse->Add(CutVar[cv].sXiPlusPiPlusbkg);
         fOutputListSparse->Add(CutVar[cv].sXiPlusPiMinusbkg);
         //
         
         
         TString *snameMCrecXiMinusPiPlus = new TString("sMCrecXiMinusPiPlus_");
         TString *snameMCrecXiPlusPiMinus = new TString("sMCrecXiPlusPiMinus_");
         *snameMCrecXiMinusPiPlus += cv;
         *snameMCrecXiPlusPiMinus += cv;
         CutVar[cv].sMCrecXiMinusPiPlus  = new THnSparseF(snameMCrecXiMinusPiPlus->Data(),"Invariant Mass Distribution", nDim,nBins,xMin,xMax);
         CutVar[cv].sMCrecXiPlusPiMinus  = new THnSparseF(snameMCrecXiPlusPiMinus->Data(),"Invariant Mass Distribution", nDim,nBins,xMin,xMax);
         fOutputListSparse->Add(CutVar[cv].sMCrecXiMinusPiPlus);
         fOutputListSparse->Add(CutVar[cv].sMCrecXiPlusPiMinus);
...
*/
    ...
    PostData(2, fOutputListL);
    PostData(3, fOutputListT);
...
void AliSigmaP::Exec(Option_t *)
{
 ...
 /*
    if (centralityV0M<0) {
        printf ("ERROR: Negative centrality");
        return;
    }
 */
   ...
   // TClonesArray *mcArray       = 0x0;
    ...
    ((TH1F*)fOutputList->FindObject("fVertexDistX"))->Fill(primaryVtx[0]);
    ((TH1F*)fOutputList->FindObject("fVertexDistY"))->Fill(primaryVtx[1]);
    ((TH1F*)fOutputList->FindObject("fVertexDistZ"))->Fill(primaryVtx[2]);
    
  //  cout<< " X === " <<primaryVtx[0]<<" ,  Y ==== " <<primaryVtx[1]<<",    Z ====" <<primaryVtx[2]<<endl;
    ...
    // Vertex systematic study default : 10 , loose : 11 , tight : 9 
        ...
        ((TH3F*)fOutputList->FindObject("fVertexDistXYZQA"))->Fill(primaryVtx[0], primaryVtx[1], primaryVtx[2]);
 ...
 /* TPC OFF */           if(nsigpi>nSigTPCPID) continue; // last update // 20140715 // TPC
  ...
  //  Double_t lRapV0 = 0;
    ...
    const AliVVertex *PrimaryVtxPi = fESD->GetPrimaryVertex();
    tPrimaryVtxPosition[0] = PrimaryVtxPi->GetX();
    tPrimaryVtxPosition[1] = PrimaryVtxPi->GetY();
    tPrimaryVtxPosition[2] = PrimaryVtxPi->GetZ();
    ...
    {   // This is the begining of the V0 loop
       ...
       // if(v0) cout<<" We have V0 " <<endl;
        ...
        cout<<" CPA = " <<lV0CosineOfPointingAngle<<endl;
        ...
        Double_t pi0TPCPID1 = 10;
        Double_t pi0TPCPID2 = 10;

        pi0TPCPID1= fPIDResponse->NumberOfSigmasTPC(pTrack,AliPID::kPion);
        pi0TPCPID2= fPIDResponse->NumberOfSigmasTPC(nTrack,AliPID::kPion);
        
         if(pi0TPCPID1>-3&&pi0TPCPID1<3) continue;
         if(pi0TPCPID2>-3&&pi0TPCPID2<3) continue;
        ...
        //if(lPosTrackCrossedRowsOverFindable<0.8) continue;
        //if(lNegTrackCrossedRowsOverFindable<0.8) continue;
        
        //End track Quality Cuts
        //________________________________________________________________________
        
       // lDcaPosToPrimVertex = TMath::Abs(pTrack->GetD(tPrimaryVtxPosition[0],tPrimaryVtxPosition[1],lMagneticField) );
        
       // lDcaNegToPrimVertex = TMath::Abs(nTrack->GetD(tPrimaryVtxPosition[0],tPrimaryVtxPosition[1],lMagneticField) );
        ...
        lAlphaV0 = v0->AlphaV0();
        lPtArmV0 = v0->PtArmV0();
        ((TH2F*)fOutputList->FindObject("hArmenterosBefore"))->Fill(lAlphaV0,lPtArmV0);

        
        if ( !(TMath::Power(lAlphaV0/0.95,2)+TMath::Power(lPtArmV0/0.05,2) < 1) ) continue;
        if(lPtArmV0>0.05) continue;
        
        ((TH2F*)fOutputList->FindObject("hArmenterosAfter"))->Fill(lAlphaV0,lPtArmV0);
        ...
        pi0Pt = sqrt((lMomPos[0]+lMomNeg[0])*(lMomPos[0]+lMomNeg[0]) + (lMomPos[1]+lMomNeg[1])*(lMomPos[1]+lMomNeg[1]));

        
        pi0p1sq=pipx1*pipx1+pipy1*pipy1+pipz1*pipz1;
        pi0p2sq=pipx2*pipx2+pipy2*pipy2+pipz2*pipz2;
        if(pi0p1sq <=0 || pi0p2sq <=0) continue;
        
        pi0e1=sqrt(pi0p1sq);
        pi0e2=sqrt(pi0p2sq);
        
        //angle=pipx1*pipx2+pipy1*pipy2+pipz1*pipz2;
        
        cout<<" P gamma 1 " <<pi0e1<<",   P gamma 1 " <<pi0e2<<",  CPA " <<lV0CosineOfPointingAngle<<endl;
        
        pi0Mass=2.*pi0e1*pi0e2*(1-lV0CosineOfPointingAngle);
        if(pi0Mass<0.) pi0Mass=1.e-8;
        pi0Mass=sqrt(pi0Mass);
        
        cout<<" MASS  " <<pi0Mass<<endl;

        
        
        ((TH1F*)fOutputList->FindObject("hPi0Mass"))->Fill(pi0Mass);

    }// This is the end of the V0 loop
    
    Int_t multClust = fESD->GetNumberOfCaloClusters();
    AliESDCaloCluster *clu1;
    TLorentzVector p1,p2,p12, pv1,pv2, pv12;
    Int_t inPHOS=0 ;

    for (Int_t i1=0; i1<multClust; i1++) {
        clu1 = fESD->GetCaloCluster(i1);
    ...
    for(Int_t icon=0; icon<nConv; icon++){
        ...
        for(Int_t icon2=0; icon2<nLam; icon2++){
            ...
            if( (fabs(arpod[1]) > 0.6 && arpod[0]< 0.12) ) {
               ...
               // if (  m > 1.230  || !( Mlamrec > 1.110 &&  Mlamrec < 1.122) ) continue;  // sa in
                
               // fSigmaMass = sig0.M();
               // fSigmaPt = sig0.Pt();
               // fSigmaArmPt = arpod[0];
               // fSigmaArmAlpha = arpod[1];
                ((TH1F*)fOutputList->FindObject("hPi0Mass1"))->Fill(m);
...
//________________________________________________________________________
void AliSigmaP::Terminate(Option_t *)
{
    cout<<"Done"<<endl;
}
//________________________________________________________________________
Double_t AliSigmaP::Det(Double_t a00, Double_t a01, Double_t a10, Double_t a11) const {// Taken from AliCascadeVertexer
    //--------------------------------------------------------------------
...
//________________________________________________________________________
Double_t AliSigmaP::Det(Double_t a00,Double_t a01,Double_t a02,
