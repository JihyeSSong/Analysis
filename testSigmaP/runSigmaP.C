void runSigmaP(int Nevents=1000, bool batchmode=kFALSE, bool MCcase=kFALSE, bool AODcase=kFALSE, const char* collectionfile="collection.xml", const char* outfilename="MyOutput.root") {
   ...
   AliCentralitySelectionTask *taskCentrality = AddTaskCentrality(); // for data 2016 April19
//    AliCentralitySelectionTask *taskCentrality = AddTaskCentrality(kFALSE); //before
    ...
    // Create tasks
    gROOT->LoadMacro("AliSigmaPEventCollection.cxx+g");
    gROOT->LoadMacro("AliSigmaP.cxx+g");
    
    AliSigmaP *myTask = new AliSigmaP("MyTask", AODcase, MCcase);
     if(!myTask) exit(-1);
    mgr->AddTask(myTask);
  ...
  //  AliAnalysisDataContainer *coutput1 = mgr->CreateContainer("MyTree", TTree::Class(),AliAnalysisManager::kOutputContainer,outfilename);
    AliAnalysisDataContainer *coutput2 = mgr->CreateContainer("MyListL", TList::Class(),AliAnalysisManager::kOutputContainer,outfilename);
    AliAnalysisDataContainer *coutput3 = mgr->CreateContainer("MyListT", TList::Class(),AliAnalysisManager::kOutputContainer,outfilename);
  ...
  //  mgr->ConnectOutput(myTask, 0, coutput1);
