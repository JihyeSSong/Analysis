...
fOutputListTree(obj.fOutputListTree),
...
AliXiStarPbPb::~AliXiStarPbPb()
{
    ...
    if(fOutputListTree) delete fOutputListTree;
 ...
 */
   ...
   // fOutputList->Add(fPhiPtDist);
    ...
    fOutputListTree = new TList();
    fOutputListTree->SetOwner();
     ...
     for(Int_t cv=0; cv<kNCutVariations; cv++){
        ...
        *nameMCrecXiPlusPiMinus += cv;
         ...
         *nameTTree += cv;
         ...
         fOutputListTree->Add(CutVar[cv].fTTree);
         ...
         *nameTTreeEMBG += cv;
         ...
         fOutputListTree->Add(CutVar[cv].fTTreeEMBG);
            ...
            for(Int_t l=0; l<(fEvt+EN)->fNTracks; l++){// Present(EN=0) and Past(EN from 1 to fEventsToMix) event track loop
                ...
                for(int cv=0; cv<kNCutVariations; cv++){
                    ...
                    if(EN==0){
                        ...
                        if(xiStarMass<2.0){
                            fTreeChargePi = fESDTrack4->Charge();
                            fTreeChargeXi = fXiTrack->Charge();
                            fTreeMass = xiStarMass;
                            fTreePt = xiStarPt;
                            fTreeCent = centralityV0M;
                            CutVar[cv].fTTree->Fill();
                        }
                        ...
                        if(xiStarMass<2.0){
                            fTreeChargePiEMBG = fESDTrack4->Charge();
                            fTreeChargeXiEMBG = fXiTrack->Charge();
                            fTreeMassEMBG = xiStarMass;
                            fTreePtEMBG = xiStarPt;
                            fTreeCentEMBG = centralityV0M;
                            CutVar[cv].fTTreeEMBG->Fill();
                        }
