...
fname(obj.fname),
//fEventHandler(obj.fEventHandler),
fAOD(obj.fAOD),
fESD(obj.fESD),
...
void AliXiStar::XiStarInit()
{
    ...
    else fEventsToMix = 1; // original 40 jisong
...
/*
    ...
    */
    ...
    TH1F *hCentralitykSelected = new TH1F("hCentralitykSelected","Centrality : after selection",100,0,100);
    ...
    hEventSelecInfo->GetXaxis()->SetBinLabel(4,"kAnyINT");
    ...
    TH1F *fCutEvents = new TH1F("fCutEvents","fCutEvents",15,0,15);
    ...
    TH3F *fXiMinusPiMinusEM = new TH3F("fXiMinusPiMinusEM","Invariant Mass Distribution", 100,0,10, 40,-2,2, 200,1.4,1.6);
    TH3F *fXiMinusPiPlusEM = new TH3F("fXiMinusPiPlusEM","Invariant Mass Distribution", 100,0,10, 40,-2,2, 200,1.4,1.6);
    TH3F *fXiPlusPiMinusEM = new TH3F("fXiPlusPiMinusEM","Invariant Mass Distribution", 100,0,10, 40,-2,2, 200,1.4,1.6);
    TH3F *fXiPlusPiPlusEM = new TH3F("fXiPlusPiPlusEM","Invariant Mass Distribution", 100,0,10, 40,-2,2, 200,1.4,1.6);
    fOutputList->Add(fXiMinusPiMinusEM);
    fOutputList->Add(fXiMinusPiPlusEM);
    fOutputList->Add(fXiPlusPiMinusEM);
    fOutputList->Add(fXiPlusPiPlusEM);
...
*/
  ...
  //  if (!isSelected) {cout<<"No event selected"<<endl; return;}
    ...
    if(!fAODcase){
         ...
         if(isSelectedMB) ((TH1F*)fOutputList->FindObject("hEventSelecInfo"))->Fill(0);
        ...
        Bool_t isSelectedC =(((AliInputEventHandler*)(AliAnalysisManager::GetAnalysisManager()->GetInputEventHandler()))->IsEventSelected() & AliVEvent::kCentral);
             if(isSelectedC) ((TH1F*)fOutputList->FindObject("hEventSelecInfo"))->Fill(1);
        ...
        Bool_t isSelectedSC =(((AliInputEventHandler*)(AliAnalysisManager::GetAnalysisManager()->GetInputEventHandler()))->IsEventSelected() & AliVEvent::kSemiCentral);
           if(isSelectedSC) ((TH1F*)fOutputList->FindObject("hEventSelecInfo"))->Fill(2);
        
        
        Bool_t isSelectedkAnyInt =(((AliInputEventHandler*)(AliAnalysisManager::GetAnalysisManager()->GetInputEventHandler()))->IsEventSelected() & AliVEvent::kAnyINT);
        if(isSelectedkAnyInt) ((TH1F*)fOutputList->FindObject("hEventSelecInfo"))->Fill(3);
        ...
        if(isSelectedkAny) ((TH1F*)fOutputList->FindObject("hEventSelecInfo"))->Fill(6);

    
        Bool_t isSelected =(((AliInputEventHandler*)(AliAnalysisManager::GetAnalysisManager()->GetInputEventHandler()))->IsEventSelected() && AliVEvent::kMB);
            if(isSelected) ((TH1F*)fOutputList->FindObject("hEventSelecInfo"))->Fill(7);
    ...
    if(centralityV0M>5.0){cout<<"Event Rejected : cent 5 - 10"<<endl; return;}
...
*/
     ...
     for(Int_t l=0; l<(fEvt)->fNTracks; l++){// Present(EN=0) and Past(EN from 1 to fEventsToMix) event track loop
                
                    if((fEvt)->fTracks[l].fID == pTrackXi->GetID()) continue;
                    if((fEvt)->fTracks[l].fID == nTrackXi->GetID()) continue;
                    if((fEvt)->fTracks[l].fID == bTrackXi->GetID()) continue;
                ...
                fESDTrack4->Set((fEvt)->fTracks[l].fX, (fEvt)->fTracks[l].fP, (fEvt)->fTracks[l].fCov, (fEvt)->fTracks[l].fCharge);
                if(fAODcase){
                    if((Bool_t)(((1<<5) & (fEvt)->fTracks[l].fFilterMap) == 0)) continue;// AOD filterbit cut, "Standard cuts with tight dca"
                }else{
                    fDecayParameters[8] = (fEvt)->fTracks[l].fDCAXY;// DCA Vtx pion third
                    ((TH1F*)fOutputList->FindObject("fDCADist_3rd_pi"))->Fill(fDecayParameters[8]);
                    
                    
                    
                    
                    if((fEvt)->fTracks[l].fDCAZ > 2) continue;
                    if( (((fEvt)->fTracks[l].fStatus)&AliESDtrack::kITSrefit)==0) continue;// Require itsrefit
                    // no Chi^2 cut applied for ESDs.  Info not available in my track structure.
                }
                
                
                if(fabs((fEvt)->fTracks[l].fEta) > 0.8) continue;
                
                fDecayParameters[3] = (fEvt)->fTracks[l].fNclusTPC;
                ((TH1F*)fOutputList->FindObject("fTPCNcls_pi3"))->Fill(fDecayParameters[3]);
                
                AliVertex *XiStarVtx = new AliVertex((fEvt)->fTracks[l].fX,0,0);
                ...
                //    if(fDecayParameters[7] < fCutValues[cv][7]) continue;// DCAVtx Lambda
                    if(cv!=8) {if(fDecayParameters[8] > (0.0105 + 0.035/pow((fEvt)->fTracks[l].fPt,1.01))) continue;}// DCAVtx pion third
                    //0.0182 + 0.035/pow((fEvt)->fTracks[l].fPt,1.01
               ...
               //     if(fDecayParameters[13] < fCutValues[cv][13]) continue;// Cos PA Lambda
                //    if(fDecayParameters[14] < fCutValues[cv][14]) continue;// Cos PA Xi
                    ...
                    if(fMCcase && mcXiFilled && !fAODcase){// ESD MC's
     ...
     //////////////////////////////////////////////////////////
     // Event Mixing ::: Reconstruct Xi(1530)
     for(Int_t EN=1; EN<fEventsToMix+1; EN++){// Event buffer loop
         
     for(Int_t ll=0; ll<(fEvt+EN)->fNTracks; ll++){// Present(EN=0) and Past(EN from 1 to fEventsToMix) event track loop
     
     fXiTrack->Set(xiVtx, xiP, fCovMatrix, Short_t(xiCharge));
     
     
     if(!fESDTrack4) continue;
     fESDTrack4->Set((fEvt+EN)->fTracks[ll].fX, (fEvt+EN)->fTracks[ll].fP, (fEvt+EN)->fTracks[ll].fCov, (fEvt+EN)->fTracks[ll].fCharge);
     
     fDecayParameters[8] = (fEvt+EN)->fTracks[ll].fDCAXY;// DCA Vtx pion third
     
     
     if((fEvt+EN)->fTracks[ll].fDCAZ > 2) continue;
     if( (((fEvt+EN)->fTracks[ll].fStatus)&AliESDtrack::kITSrefit)==0) continue;// Require itsrefit
    
     if(fabs((fEvt+EN)->fTracks[ll].fEta) > 0.8) continue;
     
     fDecayParameters[3] = (fEvt+EN)->fTracks[ll].fNclusTPC;
     
     AliVertex *XiStarVtx = new AliVertex((fEvt+EN)->fTracks[ll].fX,0,0);
     if(!(fXiTrack->PropagateToDCA(XiStarVtx, bField, 3))) continue;// Propagate tracks to dca, version which assumes fESDTrack4 is already primary
     /////////////
     fXiTrack->GetPxPyPz(pDaughter1);
     fXiTrack->GetXYZ(xDaughter1);
     ...
     if(fDecayParameters[8] > fCutValues[0][8]) continue;
    
     
   
     if(fXiTrack->Charge() == -1 &&  fESDTrack4->Charge() == -1) ((TH3F*)fOutputList->FindObject("fXiMinusPiMinusEM"))->Fill(xiStarPt, xiStarY, xiStarMass);
     else if(fXiTrack->Charge() == -1 &&  fESDTrack4->Charge() == +1) ((TH3F*)fOutputList->FindObject("fXiMinusPiPlusEM"))->Fill(xiStarPt, xiStarY, xiStarMass);
     else if(fXiTrack->Charge() == +1 &&  fESDTrack4->Charge() == -1) ((TH3F*)fOutputList->FindObject("fXiPlusPiMinusEM"))->Fill(xiStarPt, xiStarY, xiStarMass);
     else ((TH3F*)fOutputList->FindObject("fXiPlusPiPlusEM"))->Fill(xiStarPt, xiStarY, xiStarMass);
