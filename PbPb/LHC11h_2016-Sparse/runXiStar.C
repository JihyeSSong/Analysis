void runXiStar(int Nevents=1000, bool batchmode=kFALSE, bool MCcase=kFALSE, bool AODcase=kFALSE, const char* collectionfile="collection.xml", const char* outfilename="MyOutput.root") {
...
//    AliCentralitySelectionTask *taskCentrality = AddTaskCentrality(kFALSE); //before
    ...
    AliXiStarPbPb *myTask = new AliXiStarPbPb("MyTask", AODcase, MCcase);
     if(!myTask) exit(-1);
  //  myTask->SelectCollisionCandidates(AliVEvent::kCentral); //add march26 // do not work
    mgr->AddTask(myTask);
  ...
  //  AliAnalysisDataContainer *coutput1 = mgr->CreateContainer("MyTree", TTree::Class(),AliAnalysisManager::kOutputContainer,outfilename);
    AliAnalysisDataContainer *coutput2 = mgr->CreateContainer("MyListSparse", TList::Class(),AliAnalysisManager::kOutputContainer,outfilename);
