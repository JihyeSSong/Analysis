...

{
    ...
    for (Int_t cv=0; cv<kNCutVariations; cv++){
      ...
      //  CutVar[cv].fTTree=0x0;
      //  CutVar[cv].fTTreeEMBG=0x0;
...
/*fTreeMass(obj.fTreeMass),
fTreePt(obj.fTreePt),
fTreeCent(obj.fTreeCent),
fTreeChargeXi(obj.fTreeChargeXi),
fTreeChargePi(obj.fTreeChargePi),

fTreeMassEMBG(obj.fTreeMassEMBG),
fTreePtEMBG(obj.fTreePtEMBG),
fTreeCentEMBG(obj.fTreeCentEMBG),
fTreeChargeXiEMBG(obj.fTreeChargeXiEMBG),
fTreeChargePiEMBG(obj.fTreeChargePiEMBG)*/
...
AliXiStarPbPb::~AliXiStarPbPb()
{
    ...
    for (Int_t cv=0; cv<kNCutVariations; cv++){
     ...
     //   if(CutVar[cv].fTTree) delete CutVar[cv].fTTree;
     //   if(CutVar[cv].fTTreeEMBG)delete CutVar[cv].fTTreeEMBG;
...
void AliXiStarPbPb::XiStarInit()
{
 ...
 //inpPb   fMultBins = 11;// This must also be set in AliXiStarPbPb.h
  ...
  //  fMultBins = 4;// for semi - central collisions
    fMultBins = 10;// for central collisions

    
    if(fMCcase) fEventsToMix = 0;
    else fEventsToMix = 5; // original 40 jisong
   ...
   // fMultLimits[0]=0, fMultLimits[1]=150, fMultLimits[2]=300, fMultLimits[3]=450, fMultLimits[4]=600, fMultLimits[5]=750, fMultLimits[6]=900, fMultLimits[7]=1050, fMultLimits[8]=1200, fMultLimits[9]=1300, fMultLimits[10]=1400;
    ...
    fMultLimits[0]=0, fMultLimits[1]=900, fMultLimits[2]=1500, fMultLimits[3]=2200, fMultLimits[4]=3000, fMultLimits[5]=3300, fMultLimits[6]=3600, fMultLimits[7]=3900, fMultLimits[8]=4200, fMultLimits[9]=4400, fMultLimits[10]=7000;
 ...
 */
    ...
    TH1F *hCentralitySelected050 = new TH1F("hCentralitySelected050","hCentralitySelected050",100,0,100);
    hCentralitySelected050->GetXaxis()->SetTitle("Centrality");
    fOutputList->Add(hCentralitySelected050);

    TH1F *hCentralitySelected5090 = new TH1F("hCentralitySelected5090","hCentralitySelected5090",100,0,100);
   ...
   // fOutputList->Add(fPhiPtDist);
    ...
    TH2F *TPCPID = new TH2F("TPCPID","PID via TPC",1000*0.5,0,20,1000*0.5,0,200);
    fOutputList->Add(TPCPID);
    TH2F *hTPCPIDpi = new TH2F("hTPCPIDpi","PID pion",1000*0.5,0,20,1000*0.5,0,200);
    fOutputList->Add(hTPCPIDpi);
    TH2F *hTPCPIDk = new TH2F("hTPCPIDk","PID kaon",1000*0.5,0,20,1000*0.5,0,200);
    fOutputList->Add(hTPCPIDk);
    TH2F *hTPCPIDp = new TH2F("hTPCPIDp","PID proton",1000*0.5*0.5,0,20,1000*0.5,0,200);
    fOutputList->Add(hTPCPIDp);
    
    
    
    
    TH2F *hdEdxProton = new TH2F("hdEdxProton","Xi PID p",1000*0.5,0,20,1000*0.5,0,200);
    fOutputList->Add(hdEdxProton);
    TH2F *hdEdxPion1 = new TH2F("hdEdxPion1","Xi PID pi",1000*0.5,0,20,1000*0.5,0,200);
    fOutputList->Add(hdEdxPion1);
    TH2F *hdEdxPion2 = new TH2F("hdEdxPion2","Xi PID pi_b",1000*0.5,0,20,1000*0.5,0,200);
    fOutputList->Add(hdEdxPion2);
    
    TH2F *hdEdxProtonAfter = new TH2F("hdEdxProtonAfter","Xi PID p",1000*0.5,0,20,1000*0.5,0,200);
    fOutputList->Add(hdEdxProtonAfter);
    TH2F *hdEdxPion1After = new TH2F("hdEdxPion1After","Xi PID pi",1000*0.5,0,20,1000*0.5,0,200);
    fOutputList->Add(hdEdxPion1After);
    TH2F *hdEdxPion2After = new TH2F("hdEdxPion2After","Xi PID pi_b",1000*0.5,0,20,1000*0.5,0,200);
    fOutputList->Add(hdEdxPion2After);
    
    
    
    
    TH2F *hNSig3rdPion = new TH2F("hNSig3rdPion","nSigma 3rd pion",1000*0.5,0,20,1000*0.5,-5,5);
    fOutputList->Add(hNSig3rdPion);
    TH2F *hQANSig3rdPion = new TH2F("hQANSig3rdPion","nSigma 3rd pion : QA",1000*0.5,0,20,1000*0.5,-5,5);
    ...
    TH3F *fMCinputTotalXiStar1 = new TH3F("fMCinputTotalXiStar1","Invariant Mass Distribution", 100,0,10,10,0,100, 300,1.4,2.0);
    TH3F *fMCinputTotalXiStarbar1 = new TH3F("fMCinputTotalXiStarbar1","Invariant Mass Distribution",100,0,10,10,0,100, 300,1.4,2.0);
    ...
    TH3F *fMCinputTotalXiStar3 = new TH3F("fMCinputTotalXiStar3","Invariant Mass Distribution", 100,0,10,10,0,100, 300,1.4,2.0);
    TH3F *fMCinputTotalXiStarbar3 = new TH3F("fMCinputTotalXiStarbar3","Invariant Mass Distribution", 100,0,10,10,0,100, 300,1.4,2.0);
    ...
    fOutputListSparse = new TList();
    fOutputListSparse->SetOwner();

     const Int_t nDim = 3;
    Int_t nBins[nDim] = {600,100,100};
    Double_t xMin[nDim] = {1.4,0, 0};
    Double_t xMax[nDim] = {2.0,10.,100};
     ...
     for(Int_t cv=0; cv<kNCutVariations; cv++){
        ...
        *nameXiPlusPiMinusbkg += cv;
        CutVar[cv].fXiMinusPiPlus  = new TH3F(nameXiMinusPiPlus->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 300,1.4,2.0);
        CutVar[cv].fXiMinusPiMinus = new TH3F(nameXiMinusPiMinus->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 300,1.4,2.0);
        CutVar[cv].fXiPlusPiPlus   = new TH3F(nameXiPlusPiPlus->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 300,1.4,2.0);
        CutVar[cv].fXiPlusPiMinus  = new TH3F(nameXiPlusPiMinus->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 300,1.4,2.0);
        CutVar[cv].fXiMinusPiPlusbkg  = new TH3F(nameXiMinusPiPlusbkg->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 300,1.4,2.0);
        CutVar[cv].fXiMinusPiMinusbkg = new TH3F(nameXiMinusPiMinusbkg->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 300,1.4,2.0);
        CutVar[cv].fXiPlusPiPlusbkg   = new TH3F(nameXiPlusPiPlusbkg->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 300,1.4,2.0);
        CutVar[cv].fXiPlusPiMinusbkg  = new TH3F(nameXiPlusPiMinusbkg->Data(),"Invariant Mass Distribution",100,0,10,10,0,100, 300,1.4,2.0);
        ...
        *nameMCrecXiPlusPiMinus += cv;
        CutVar[cv].fMCrecXiMinusPiPlus  = new TH3F(nameMCrecXiMinusPiPlus->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 300,1.4,2.0);
        CutVar[cv].fMCrecXiPlusPiMinus  = new TH3F(nameMCrecXiPlusPiMinus->Data(),"Invariant Mass Distribution", 100,0,10,10,0,100, 300,1.4,2.0);
        ...
        /* TString *nameTTree = new TString("fTTree_");
         *nameTTree += cv;
         CutVar[cv].fTTree  = new TTree(nameTTree->Data(),"TTree output for pT, centrality, mass");
         CutVar[cv].fTTree->Branch("fTreeMass",&fTreeMass,"fTreeMass/F");
         CutVar[cv].fTTree->Branch("fTreePt",&fTreePt,"fTreePt/F");
         CutVar[cv].fTTree->Branch("fTreeCent",&fTreeCent,"fTreeCent/F");
         CutVar[cv].fTTree->Branch("fTreeChargeXi",&fTreeChargeXi,"fTreeChargeXi/F");
         CutVar[cv].fTTree->Branch("fTreeChargePi",&fTreeChargePi,"fTreeChargePi/F");
         fOutputListSparse->Add(CutVar[cv].fTTree);
         
         TString *nameTTreeEMBG = new TString("fTTreeEMBG_");
         *nameTTreeEMBG += cv;
         CutVar[cv].fTTreeEMBG  = new TTree(nameTTreeEMBG->Data(),"TTree output for pT, centrality, mass for event mixing bg");
         CutVar[cv].fTTreeEMBG->Branch("fTreeMassEMBG",&fTreeMassEMBG,"fTreeMassEMBG/F");
         CutVar[cv].fTTreeEMBG->Branch("fTreePtEMBG",&fTreePtEMBG,"fTreePtEMBG/F");
         CutVar[cv].fTTreeEMBG->Branch("fTreeCentEMBG",&fTreeCentEMBG,"fTreeCentEMBG/F");
         CutVar[cv].fTTreeEMBG->Branch("fTreeChargeXiEMBG",&fTreeChargeXiEMBG,"fTreeChargeXiEMBG/F");
         CutVar[cv].fTTreeEMBG->Branch("fTreeChargePiEMBG",&fTreeChargePiEMBG,"fTreeChargePiEMBG/F");
         fOutputListSparse->Add(CutVar[cv].fTTreeEMBG);
        */
...
void AliXiStarPbPb::Exec(Option_t *)
{
  ...
  //  fMCcase = kTRUE;
    ...
    if(!(isSelectedkCentral|isSelectedkSemiCentral))((TH1F*)fOutputList->FindObject("hCentralitySelected050"))->Fill(centralityV0M);
    if(centralityV0M>50. && centralityV0M<90.)((TH1F*)fOutputList->FindObject("hCentralitySelected5090"))->Fill(centralityV0M);
    
    if(!fMCcase){
       // if(!(isSelectedMB|isSelectedkCentral|isSelectedkSemiCentral)) {cout<<"Event Rejected"<<endl; return;}
       // if(!isSelectedkALL) {cout<<"Event Rejected"<<endl; return;}
         if(!(isSelectedkCentral|isSelectedkSemiCentral)) {cout<<"Event Rejected"<<endl; return;}
       // if(!(isSelectedkCentral|isSelectedkSemiCentral|(centralityV0M>50. && centralityV0M<90.))) {cout<<"Event Rejected"<<endl; return;}
                            ...
                            if(MCXiesd->GetMother(0) == MCXiStarD2esd->GetMother(0)){
                                ...
                                if(abs(MCXiStaresd->GetPdgCode())==kXiStarCode) {
                                    ...
                                    if(fXiTrack->Charge() == -1 &&  fESDTrack4->Charge() == +1) {
                                        ...
                                        CutVar[cv].sMCrecXiMinusPiPlus->Fill(sparesFill,1);
                                    }
                                    if(fXiTrack->Charge() == +1 &&  fESDTrack4->Charge() == -1) {
                                        Double_t sparesFill[4] = {xiStarMass, xiStarPt, centralityV0M};
                                        CutVar[cv].fMCrecXiPlusPiMinus->Fill(xiStarPt, centralityV0M, xiStarMass);
                                        CutVar[cv].sMCrecXiPlusPiMinus->Fill(sparesFill,1);
                                    }
...
Double_t AliXiStarPbPb::ApplyCentralityPatchPbPb2011(){
    ...
    fUseCentralityPatchPbPb2011 = 510;
...
Double_t AliXiStarPbPb::ApplyCentFlatPbPb2011(){
    //This part rejects randomly events such that the centrality gets flat for LHC11h Pb-Pb data
    //for 0-10% centrality bin
    ...
    Double_t cent = (Float_t)(centrality->GetCentralityPercentile("V0M"));
    Double_t rnd_hc = -1., testf = 0.0, N2 = -1.;
    
     if (!fMCcase){
    N2 = 1.662323e+06; //N2 is the reference
    if(cent>0.&&cent<=1.)testf = N2/2.083619e+06;
    if(cent>1.&&cent<=2.)testf = N2/2.142350e+06;
    if(cent>2.&&cent<=3.)testf = N2/2.187767e+06;
    if(cent>3.&&cent<=4.)testf = N2/2.195456e+06;
    if(cent>4.&&cent<=5.)testf = N2/2.191028e+06;
    if(cent>5.&&cent<=6.)testf = N2/2.132944e+06;
    if(cent>6.&&cent<=7.)testf = N2/2.153071e+06;
    if(cent>7.&&cent<=8.)testf = N2/2.113802e+06;
    if(cent>8.&&cent<=9.)testf = N2/2.024300e+06;
    if(cent>9.&&cent<=10.)testf = N2/1.662323e+06;
     }
    if (fMCcase){
        N2 = 1.6e+05; //N2 is the reference
        
        if(cent>0.&&cent<=1.)testf = N2/2.31867e+05;
        if(cent>1.&&cent<=2.)testf = N2/1.74437e+05;
        if(cent>2.&&cent<=3.)testf = N2/1.82509e+05;
        if(cent>3.&&cent<=4.)testf = N2/1.85920e+05;
        if(cent>4.&&cent<=5.)testf = N2/1.87276e+05;
        if(cent>5.&&cent<=6.)testf = N2/1.80858e+05;
        if(cent>6.&&cent<=7.)testf = N2/1.75829e+05;
        if(cent>7.&&cent<=8.)testf = N2/1.58283e+05;
        if(cent>8.&&cent<=9.)testf = N2/1.34447e+05;
        if(cent>9.&&cent<=10.)testf = N2/1.00219e+05;
    }
...
Double_t AliXiStarPbPb::ComputeCentrality()
{
    ...
    SelecCent=centrality->GetCentralityPercentile("V0M");
        ...
        if(SelecCent>0. && SelecCent<10.) //  Flat applied
       //return ApplyCentralityPatchPbPb2011();
        return ApplyCentFlatPbPb2011();
