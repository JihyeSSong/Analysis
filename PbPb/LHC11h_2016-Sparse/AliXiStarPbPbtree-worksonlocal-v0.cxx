...

{
    ...
    for (Int_t cvt=0; cvt<kNCutVariations; cvt++){
        CutVarTree[cvt].fTTree=0x0;
        CutVarTree[cvt].fTTreeEMBG=0x0;
    }
...
AliXiStarPbPb::~AliXiStarPbPb()
{
    ...
    for (Int_t cvt=0; cvt<kNCutVariations; cvt++){
        if(CutVarTree[cvt].fTTree) delete CutVarTree[cvt].fTTree;
        if(CutVarTree[cvt].fTTreeEMBG)delete CutVarTree[cvt].fTTreeEMBG;
...
void AliXiStarPbPb::XiStarInit()
{
 ...
 //inpPb   fMultBins = 11;// This must also be set in AliXiStarPbPb.h
  ...
  //  fMultBins = 4;// for semi - central collisions
      fMultBins = 10;// for MB collisions
   ...
   // fMultLimits[0]=0, fMultLimits[1]=5000, fMultLimits[2]=10000, fMultLimits[3]=15000, fMultLimits[4]=20000; // for semi-central collisions
    fMultLimits[0]=0, fMultLimits[1]=100, fMultLimits[2]=250, fMultLimits[3]=600, fMultLimits[4]=1000, fMultLimits[5]=2500, fMultLimits[6]=4000, fMultLimits[7]=6000, fMultLimits[8]=9000, fMultLimits[9]=16000, fMultLimits[10]=20000;
 ...
 */
    ...
    hEventSelecInfo->GetXaxis()->SetBinLabel(5,"kALL");
   ...
   // fOutputList->Add(fPhiPtDist);
    ...
    for(Int_t cvt=0; cvt<kNCutVariations; cvt++){
        ...
        *nameTTree += cvt;
        CutVarTree[cvt].fTTree  = new TTree(nameTTree->Data(),"TTree output for pT, centrality, mass");
        CutVarTree[cvt].fTTree->Branch("fTreeMass",&fTreeMass,"fTreeMass/F");
        CutVarTree[cvt].fTTree->Branch("fTreePt",&fTreePt,"fTreePt/F");
        CutVarTree[cvt].fTTree->Branch("fTreeCent",&fTreeCent,"fTreeCent/F");
        CutVarTree[cvt].fTTree->Branch("fTreeChargeXi",&fTreeChargeXi,"fTreeChargeXi/F");
        CutVarTree[cvt].fTTree->Branch("fTreeChargePi",&fTreeChargePi,"fTreeChargePi/F");
        fOutputListTree->Add(CutVarTree[cvt].fTTree);
        
        TString *nameTTreeEMBG = new TString("fTTreeEMBG_");
        *nameTTreeEMBG += cvt;
        CutVarTree[cvt].fTTreeEMBG  = new TTree(nameTTreeEMBG->Data(),"TTree output for pT, centrality, mass for event mixing bg");
        CutVarTree[cvt].fTTreeEMBG->Branch("fTreeMassEMBG",&fTreeMassEMBG,"fTreeMassEMBG/F");
        CutVarTree[cvt].fTTreeEMBG->Branch("fTreePtEMBG",&fTreePtEMBG,"fTreePtEMBG/F");
        CutVarTree[cvt].fTTreeEMBG->Branch("fTreeCentEMBG",&fTreeCentEMBG,"fTreeCentEMBG/F");
        CutVarTree[cvt].fTTreeEMBG->Branch("fTreeChargeXiEMBG",&fTreeChargeXiEMBG,"fTreeChargeXiEMBG/F");
        CutVarTree[cvt].fTTreeEMBG->Branch("fTreeChargePiEMBG",&fTreeChargePiEMBG,"fTreeChargePiEMBG/F");
        fOutputListTree->Add(CutVarTree[cvt].fTTreeEMBG);
