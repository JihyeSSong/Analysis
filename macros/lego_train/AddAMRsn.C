...
#endif
Bool_t AddAMRsn(TString analysisSource = "local", TString analysisMode = "full",TString input="esd",TString inputMC="", TString postfix = "",TString idStr="0")
{
...
//   useMixing = kTRUE;
   ...
   // Rsn Particle
   // TString rsnStr="Phi"; jisong
    TString rsnStr="Lambda"; //jisong
    
    
   // Rsn Cut
   TString rsnCutStr="";

   //rsnCutStr="PhiNsigma:KTPCnsig30"; jisong
    rsnCutStr="LambdaNsigma:KTPCsig30"; //jisong

   if ((rsnCutStr.IsNull())&&(!postfix.IsNull())) {
     //  rsnCutStr = "PhiNsigma:"; jisong
       rsnCutStr = "LambdaNsigma:"; 
...
//   extraMacroArgs = "10, 5, 5, -1, 1, 0, 1, 1, 1";
   ...
   if (usePrivateTrain) {
      ...
      if (gSystem->Getenv("ALICE_ROOT")) {
         ...
         if (alirootVersion<"v5-02-19-AN" && alirootVersion.CompareTo("trunk")) rsnLibName = "PWGLFresonances";
